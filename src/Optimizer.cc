...
void Optimizer::GlobalBundleAdjustemnt(MapPtr  pMap, int nIterations, unsigned long fixId,  bool* pbStopFlag, const unsigned long nLoopKF, const bool bRobust)
{
    vector<KeyFramePtr > vpKFs = pMap->GetAllKeyFrames();
    vector<MapPointPtr > vpMP = pMap->GetAllMapPoints();
    BundleAdjustment(vpKFs,vpMP, fixId, nIterations,pbStopFlag, nLoopKF, bRobust);
}


void Optimizer::BundleAdjustment(const vector<KeyFramePtr > &vpKFs, const vector<MapPointPtr > &vpMP,
                                 unsigned long fixId, int nIterations, bool* pbStopFlag, const unsigned long nLoopKF, const bool bRobust)
{

    cout << S::red << __FUNCTION__;
    ...
    for(size_t i=0; i<vpMP.size(); i++)
    {
        ...
        for(map<KeyFramePtr ,size_t>::const_iterator mit=observations.begin(); mit!=observations.end(); mit++)
        {
            ...
            int viewedkpCam = pKF->keypointToCam[viewedkpId];
            if (viewedkpCam != 0)  {
                nCrossCamEdges++;
            }

            const cv::KeyPoint &kpUn = pKF->mvTotalKeysUn[mit->second];
            ...
            e->fx = pKF->mvfx[viewedkpCam];
            e->fy = pKF->mvfy[viewedkpCam];
            e->cx = pKF->mvcx[viewedkpCam];
            e->cy = pKF->mvcy[viewedkpCam];

            cv::Mat camExt = pKF->mpCameras->getExtrinsici(viewedkpCam);  // 相机系统到相机i的外参
            cv::Mat camExtAdj = pKF->mpCameras->getExtrinsicAdji(viewedkpCam);

            g2o::SE3Quat ExtSE3 = Converter::toSE3Quat(camExt);
            Eigen::Matrix<double,6,6> ExtAdj = Converter::toMatrix6d(camExtAdj);

            e->setExtrinsic(ExtSE3, ExtAdj);
    ...
    cout << " cross camera edges: " << nCrossCamEdges << S::endc << endl;
    // Optimize!
...

    {
        ...
        for(int i=0; i<N; i++)
        {
            ...
            if(pMP)
            {
                ...
                Eigen::Matrix<double,2,1> obs;
                const cv::KeyPoint &kpUn = pFrame->mvTotalKeysUn[i];
                int cam = pFrame->keypointToCam[i];
                obs << kpUn.pt.x, kpUn.pt.y;
                ...
                e->fx = pFrame->mvfx[cam];
                e->fy = pFrame->mvfy[cam];
                e->cx = pFrame->mvcx[cam];
                e->cy = pFrame->mvcy[cam];
                ...
                cv::Mat camExt = pFrame->mvExtrinsics[cam];  // 相机系统到相机i的外参
                cv::Mat camExtAdj = pFrame->mvExtAdj[cam];
...
void Optimizer::LocalBundleAdjustment(KeyFramePtr pKF, bool* pbStopFlag, MapPtr  pMap, size_t fixId = 0)
{    
    ...
    unordered_map<size_t, vector<int> > MP_Cams;
    ...
    for(list<MapPointPtr >::iterator lit=lLocalMapPoints.begin(), lend=lLocalMapPoints.end(); lit!=lend; lit++)
    {
        ...
        for(map<KeyFramePtr ,size_t>::const_iterator mit=observations.begin(), mend=observations.end(); mit!=mend; mit++)
        {
            ...
            if(!pKFi->isBad())
            {
                const cv::KeyPoint &kpUn = pKFi->mvTotalKeysUn[mit->second];
                size_t viewedkpId = mit->second;
                int viewedkpCam = pKFi->keypointToCam[viewedkpId];

                MP_Cams[pMP->mnId].push_back(viewedkpCam);
                ...
                e->fx = pKFi->mvfx[viewedkpCam];
                e->fy = pKFi->mvfy[viewedkpCam];
                e->cx = pKFi->mvcx[viewedkpCam];
                e->cy = pKFi->mvcy[viewedkpCam];
                cv::Mat camExt = pKF->mpCameras->getExtrinsici(viewedkpCam);  // 相机系统到相机i的外参
                cv::Mat camExtAdj = pKF->mpCameras->getExtrinsicAdji(viewedkpCam);
    ...
    // 统计一下本次local BA 中跨了两个相机的 MP 个数
    /*
    int total = 0;
    for(auto it = MP_Cams.begin(); it != MP_Cams.end(); it++) {
        size_t MPid = it->first;
        vector<int> viewedCams = it->second;
        int nViewed = viewedCams.size();
        int count = 0;
        for(int i = 0; i < nViewed; i++) {
            count += viewedCams[i];
        }
        // 如果全被0相机看到， 则count = 0;
        // 如果全被1相机看到， 则count = nViewed × 1
        if(count != 0 && count != nViewed)  total++;
    }
    if(total != 0)
        cout << S::purple << "Optimizer::" << __FUNCTION__ << " Crossed nMPs" << total  << S::endc << endl;
    */
    ...
    //cout << S::purple << "Optimizer::" << __FUNCTION__ << " finished" << S::endc << endl;
}


void Optimizer::OptimizeEssentialGraph(MapPtr  pMap,
                                       KeyFramePtr  pLoopKF,
                                       KeyFramePtr  pCurKF,
...
int Optimizer::OptimizeSim3(KeyFramePtr pKF1,
                            const int& queryC,
                            KeyFramePtr pKF2,
                            const int& respC,
                            ...
                            const bool bFixScale)
{
    ...
    // Calibration
    const cv::Mat &K1 = pKF1->mvK[queryC];
    const cv::Mat &K2 = pKF2->mvK[respC];

    // Camera poses
    const cv::Mat R1w = pKF1->GetRotation(queryC);
    const cv::Mat t1w = pKF1->GetTranslation(queryC);
    const cv::Mat R2w = pKF2->GetRotation(respC);
    const cv::Mat t2w = pKF2->GetTranslation(respC);
    ...
    for(int i=0; i<N; i++)
    {
        ...
        const cv::KeyPoint &kpUn1 = pKF1->mvTotalKeysUn[i];
        ...
        const cv::KeyPoint &kpUn2 = pKF2->mvTotalKeysUn[i2];
